...
{ name: 'Squarespace', pattern: /https://www.google.com/search?q=static.squarespace.com/i },
...
{ name: 'Stripe', pattern: /https://www.google.com/search?q=stripe.com/v3/i },
],
framework: [
...
{ name: 'Next.js', pattern: /NEXT_DATA/i },
...
{ name: 'Hotjar', pattern: /https://www.google.com/search?q=hotjar.com/i },
{ name: 'Segment', pattern: /https://www.google.com/search?q=segment.com/i },
]
};
...
const targetUrl = https://api.hackertarget.com/findshareddns/?q=${cleanNs}; try { const res = await fetch(https://corsproxy.io/?${encodeURIComponent(targetUrl)});
...
<span
onClick={onClick}
className={text-xs px-2 py-0.5 rounded border ${colors[color] || colors.blue} font-medium whitespace-nowrap flex items-center gap-1 transition-colors ${onClick ? 'cursor-pointer' : ''} ${className}}
>
{children}
</span>
);
...
return (
<div className={w-6 h-6 flex items-center justify-center rounded font-bold text-xs border shadow-sm ${colors[grade] || colors.F}}>
{grade}
</div>
);
};
...
<div className={w-12 h-12 flex items-center justify-center rounded-lg text-2xl font-bold border-2 ${ subdomain.security?.grade === 'A' ? 'border-green-500 text-green-500' : subdomain.security?.grade === 'F' ? 'border-red-500 text-red-500' : 'border-yellow-500 text-yellow-500' }}>
...
<input type="text" className="w-full bg-slate-800 border border-slate-700 rounded-xl px-6 py-4 text-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" placeholder="Enter root domain (e.g. https://www.google.com/search?q=google.com)" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
